...
def interactive_spectrum_viewer(galaxy_index=0):
    ...
    def mark_attention(button):
        flag_dropdown.value = "0"; catalog_filtered.loc[galaxy_index, "Redshift"] = -2 
        #comments_box.value = "Uncertain solution, flagged for review"
        catalog_filtered.loc[galaxy_index, "Comments"] = comments_box.value + ". Uncertain solution, flagged for review"
        update_viewer()
    ...
    def save_current_state():
        # Use the specific row at galaxy_index instead of finding all matches by galaxy_id
        catalog_filtered.loc[galaxy_index, "Comments"] = comments_box.value
        catalog_filtered.loc[galaxy_index, "Flag"] = flag_dropdown.value
        if not flag_dropdown.value:
            catalog_filtered.loc[galaxy_index, "Redshift"] = -99  
        for cb, feature in zip(checkboxes, features):
            catalog_filtered.loc[galaxy_index, feature] = 1 if cb.value else 0
    ...
    def on_redshift_change(change):
        # Update only the specific row at galaxy_index
        catalog_filtered.loc[galaxy_index, "Redshift"] = change["new"]
        update_viewer()

    def on_flag_change(change):
        # Update only the specific row at galaxy_index
        catalog_filtered.loc[galaxy_index, "Flag"] = change["new"]
